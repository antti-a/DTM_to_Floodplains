...
THE ALGORITHM
    ...
    Each pixel is visited exactly once, so the run time is O(n pixels).
    Flow directed at NoData pixels or off the grid edge leaves the
    domain. Pixels caught in a directed cycle (should not occur in a
    well-formed flow-direction raster) are left with partial
    accumulation and returned in the unresolved count.

INPUT CONVENTION
    ``frac`` holds, for each pixel, the fraction of its outflow sent to
    each of its 8 neighbours in the canonical order N, NE, E, SE, S,
    SW, W, NW - the band order of the MFD / MD-infinity rasters and of
    the ``DROW`` / ``DCOL`` offsets below. A pixel whose 8 fractions are
    all zero is a sink.
...
@njit(cache=True)
def accumulate(frac, valid, pixel_area, drow, dcol):
    ...
    Returns (accumulation float64 (rows, cols), number of unprocessed pixels).
    """
    rows, cols, _ = frac.shape
    acc = np.zeros((rows, cols), dtype=np.float64)
    indeg = np.zeros((rows, cols), dtype=np.int32)

    # In-degree = number of valid donors draining into each pixel.
    for r in range(rows):
        for c in range(cols):
            if not valid[r, c]:
                continue
    ...
    # Seed the queue with pixels that receive no inflow (local maxima).
    ...
    # Pop pixels whose upstream area is complete and push it downstream.
    head = 0
    processed = 0
    while head < tail:
        idx = queue[head]
