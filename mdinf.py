...
THE ALGORITHM - MDinf, Seibert and McGlynn (2007)
    ...
    the cell's flow proportional to its slope raised to an exponent p
...
OUTPUT CONVENTION
    ...
    read identically. Nodata cells, and valid cells with no downslope
    ...
    than rerouted, so next to flats and nodata a cell's fractions can sum
    to slightly less than one.

    Two deliberate deviations from the Rust, neither affecting a
    conditioned DEM: the facet-aspect buffer is reset for every cell
    (WhiteboxTools reuses it, letting stale angles from previously
    processed cells leak into edge cases beside nodata), and undefined
    cells are NaN rather than silently accumulating nothing.
"""
...
def _mdinf_weights(dem, nodata, grid_res, exponent):
    """Per-cell MDinf flow fractions, WhiteboxTools facet order.
        ...
        for col in range(cols):
            ...
            for i in range(8):
                ...
                if p1 != nodata and p2 != nodata:
                    z1 = p1 - z
                    z2 = p2 - z
                    # Normal to the facet through the cell and both neighbours.
...
def mdinf_flowdir(dem, nodata, dx, dy, exponent=1.1):
    ...
    nodata its nodata value, dx/dy the cell size in map units, exponent
    the facet-slope dispersion exponent p. Returns float64 fractions in
    band order N, NE, E, SE, S, SW, W, NW; NaN bands mark nodata cells
    and cells with no downslope facet. See the module docstring for the
