# DTM_to_Floodplains — geomorphic floodplains for Finland from the 2 m national DEM

Maps **geomorphic floodplains** (GFPLAIN; Nardi et al., 2019) and **height
above nearest drain** (HAND; Nobre et al., 2016) directly from Finland's
national 2 m elevation model (KM2) — terrain analysis only, no hydraulic
modelling. The pipeline is built for Finnish data end to end — the DEM is
first carved with SYKE's national culvert-correction layer so flow crosses
road embankments instead of ponding behind them, then conditioned, routed
and accumulated into HAND and floodplain rasters — but it can be modified
to work in other areas by swapping or skipping the culvert-carving stage.
...
0 = upland) plus every intermediate product, including `data/05_hand/hand.tif`.

## Running the pipeline

**Full run** — all six stages, stops at the first failure:
...
only parametrized step (HAND has no parameters and needs no rerun). How to
choose `a` and `b` depends on the application; trying values is cheap,
because only stage 6 reruns (seconds — stages 1–5 are untouched):

```bash
python 06_floodplains.py --a 0.8
python 06_floodplains.py --a 1.0 --b 0.35
```
...
**Compare routing algorithms** (optional extra — not needed for
...
| `03_flow_router.py` | `--fdir d8 mfd dinf mdinf` / `all` | routing algorithms to run (d8 — what stages 4–6 consume) |
...
4. **Accumulate** — weighted flow accumulation (upstream contributing
   area) for every flow-direction raster found; required by stages 5–6.
...
which is useful in practice during calibration: any raster can always tell
you which inputs and parameter values produced it.
...
and their Numba kernels are adapted from `pyflwdir.dem` (compiled for
whole-DEM grids): HAND after Nobre et al. (2016), and GFPLAIN after
